...
if params['instance_seg'] == 'embedding':
    inst_loss_fn = SpatialEmbLoss(n_sigma=2, to_center=True, foreground_weight=10, H=256,W=256).to(device)
...
color_aug_fn = color_augmentations(197, s=0.5)
validation_loss = []
step = -1
        ...
        for raw, gt in tmp_loader:
            ...
            if step % params['snapshot_step'] == 0:
                ...
                if params['instance_seg'] == 'embedding':
                    ...
                    xym_s = emb_loss_fn.xym[:, 0:h, 0:w].contiguous()
                    spatial_emb = pred_inst[0, 0:2] + xym_s  # 2 x h x w
                    sigma = pred_inst[0, 2:2+emb_loss_fn.n_sigma]  # n_sigma x h x w
                    seed_map = torch.sigmoid(
                        pred_inst[0, 2+emb_loss_fn.n_sigma:2+emb_loss_fn.n_sigma + 1])  # 1 x h x w
